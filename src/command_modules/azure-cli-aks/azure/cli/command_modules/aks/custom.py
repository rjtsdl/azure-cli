...
def aks_browse(client, resource_group_name, name, disable_browser=False):
    ...
    proxy_url = 'http://127.0.0.1:8001/ui'
...
def aks_create(client, resource_group_name, name, ssh_key_value,  # pylint: disable=too-many-locals
    ...
    _handle_response(response, no_wait, name, operation="\nCreate", success_codes=[200, 201])


def aks_delete(client, resource_group_name, name, no_wait=False, **_):
...
def aks_scale(client, resource_group_name, name, agent_count, no_wait=False):
    ...
    return _handle_response(response, no_wait, name, operation="Scale", success_codes=[200, ])


def aks_upgrade(client, resource_group_name, name, kubernetes_version, no_wait=False, **_):
    ...
    response = client.create_or_update(resource_group_name, name, instance, raw=no_wait)
    return _handle_response(response, no_wait, name, operation="upgrade", success_codes=[200, ])
...
def store_aks_service_principal(subscription_id, client_secret, service_principal,
...
def _ensure_service_principal(service_principal=None,
    ...
    if not service_principal:
        ...
        if principalObj:
            ...
            store_aks_service_principal(subscription_id, client_secret, service_principal)
...
def _handle_response(response, no_wait, name, operation, success_codes):
    ...
    if no_wait:
        ...
        if success_codes and raw.status_code in success_codes:
